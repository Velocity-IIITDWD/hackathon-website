...
export const SponsorCard: React.FC<SponsorCardProps> = ({ id, name, website, logo, socialLinks }) => {
  const Bg = () => {
    return (
      <svg id={`${id}-svg`} className='size-full -z-50' width="550" height="308" viewBox="0 0 550 308" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g opacity="0.5">
        ...
        <path id={`${id}-svgPath`} opacity="0.8" d="M10.5812 8.0001L9.58118 7.99809L9.57918 8.99809L9.06433 265.716L9.06342 266.173L9.40861 266.473L46.4292 298.625L46.7105 298.87L47.0829 298.87L538.958 299.857L539.958 299.859L539.96 298.859L540.475 42.1416L540.476 41.6844L540.131 41.3846L503.11 9.23154L502.829 8.9873L502.456 8.98655L10.5812 8.0001ZM15.393 13.3164L500.49 14.2892L534.649 43.9568L534.146 294.541L49.0493 293.568L14.8905 263.9L15.393 13.3164Z" fill="#0088BE" stroke="#0088BE" strokeWidth="0" />
      </svg>
    )
  }
  ...
  return (
    <div className="relative w-80 h-[179.2px]">
      ...
      <div id={`${id}-container`} className='flex gap-8 justify-center items-center size-full'>
        <div id={`${id}-logoContainer`} className="select-none w-20 h-20 flex justify-center items-center rounded-full shadow-md relative">
          <Image height={80} width={80} src={logo} alt={`${name} logo`} className="opacity-100 z-30 w-full h-full object-cover absolute top-0 left-0 rounded-full" />
          <Image height={80} width={80} src={logo} alt={`${name} logo`} className="opacity-80 z-20 w-full h-full object-cover absolute top-0 left-0 rounded-full" />
          <Image height={80} width={80} src={logo} alt={`${name} logo`} className="opacity-60 z-10 w-full h-full object-cover absolute top-0 left-0 rounded-full" />
        ...
        <div id={`${id}-contentContainer`} className="z-0 flex flex-col items-center gap-4 text-center size-fit">
          <div className='z-10'>
